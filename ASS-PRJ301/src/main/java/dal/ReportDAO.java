...
public class ReportDAO {
    ...
    public List<Object[]> getSalesByEmployeeAndBook() {
        ...
        try {
            TypedQuery<Object[]> query = em.createQuery(
                "SELECT u.fullName, b.title, SUM(od.quantity), SUM(od.lineTotal) " +
                "FROM OrderDetail od " +
                "JOIN od.order o " +
                "JOIN o.cashierUser u " +
                "JOIN od.book b " +
                "WHERE o.status = 'PAID' " +
                "GROUP BY u.id, u.fullName, b.title " +
"ORDER BY u.fullName, SUM(od.lineTotal) DESC", Object[].class);
            return query.getResultList();
        } finally {
            if (em != null && em.isOpen()) {
                em.close();
